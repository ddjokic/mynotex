...
begin
  ...
  try
    if Assigned(pixbuf) then
      gdk_pixbuf_save(pixbuf, PChar(FileName), 'jpeg', nil);
  except
    Result := False;
  end;
end;
...
var
  ...
  list: PGList;
  buffer: PGtkTextBuffer;
  istart, iBeginning, iend: TGtkTextIter;
  stPar: string;
begin
  // Added by Massimo Nardello
  ...
  if not Assigned(TextWidget) then
    Exit;
  buffer := gtk_text_view_get_buffer(PGtkTextView(TextWidget));
  gtk_text_buffer_get_iter_at_line(buffer, @iBeginning, CurrLine);
  gtk_text_buffer_get_iter_at_line(buffer, @iStart, CurrLine);
  ...
  while ((gtk_text_iter_ends_line(@iEnd) = False) and
    ...
    gtk_text_iter_forward_char(@iEnd);
  gtk_text_iter_forward_char(@iEnd);
  stPar := gtk_text_iter_get_slice(@iStart, @iEnd);
  gtk_text_buffer_delete(buffer, @iStart, @iEnd);
  if MoveUp = True then
    gtk_text_iter_backward_line(@iStart)
  else
    gtk_text_iter_forward_line(@iStart);
  gtk_text_buffer_insert(buffer, @iStart, PChar(stPar), -1);
end;
